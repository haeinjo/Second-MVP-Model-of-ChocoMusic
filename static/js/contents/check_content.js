const checkPhoto = document.querySelector(".check__element.photo");
const checkTitle = document.querySelector(".check__element.title");
const checkGenre = document.querySelector(".check__element.genre");
const checkProject = document.querySelector(".check__element.project");
const checkRadio = document.querySelector(".check__radio");
const submitCheck = document.querySelector(".check__button");
let setExposureLevel = false;


window.onload = initContentCheck();

function initContentCheck() {
    setClickRadioInput();
    submitCheck.addEventListener("click", clickSubmitBtn);
}

function setCheckPhoto() {
    if (photoInput.files) {
        checkPhoto.innerHTML = "";
        const newImg = document.createElement("img");
        const maskPhoto = photoInput.files[0];

        newImg.file = maskPhoto;
        newImg.classList.add("img--added");
        checkPhoto.appendChild(newImg);

        const reader = new FileReader();
        reader.onload = (function (aImg) { return function (e) { aImg.src = e.target.result; }; })(newImg);
        reader.readAsDataURL(maskPhoto);
    }
}

function setCheckTitle() {
    const thisTitle = titleInput.value;
    checkTitle.innerText = thisTitle;
}

function setCheckGenre() {
    const thisGenre = genreInput.selectedOptions[0].text;
    checkGenre.innerText = thisGenre;
}

function setCheckProject() {
    const thisProject = projectInput.selectedOptions[0].text;
    checkProject.innerText = thisProject;
}

function setClickRadioInput() {
    const radioInputs = checkRadio.querySelectorAll("input");
    for (let i = 0; radioInputs[i]; ++i) {
        radioInputs[i].addEventListener("click", clickRadioInput);
    }
}

function clickRadioInput(e) {
    const selectedValue = e.target.value;
    const selectedInput = exposureLevelInput.querySelector(`[value="${selectedValue}"]`);
    selectedInput.click();
    setExposureLevel = true;
}

function clickSubmitBtn() {
    if (setExposureLevel === true) {
        formButton.click();
    } else {
        // 에러 메세지 출력
    }
